 ...
 *
 * Examples:
 * - A tech newsletter → tweet_draft ("Share your take on this trend")
 ...
 * - A local event email → place_to_visit ("Check out this pottery class in Shorewood")
 * - An industry newsletter → learning ("Dive into this RAG tutorial they linked")
 * - A product launch email → tool_to_try ("This scheduling tool could replace Calendly for you")
 */
export const IDEA_TYPES = [
  'tweet_draft',       // A specific tweet or social post — the actual draft, not "write a tweet about X"
  ...
  'learning',          // Something to learn — course, tutorial, book, skill, concept to explore
  'tool_to_try',       // A tool, app, or service mentioned that's worth trying
  'place_to_visit',    // Restaurant, cafe, event, attraction, experience to check out
  'date_night',        // Partner/relationship activities, romantic ideas
  'family_activity',   // Activities with kids, family outings, traditions
  'personal_growth',   // Habits to build, routines to start, health/wellness ideas
 ...
 * REFINED (Mar 2026): 0-3 ideas (was always 3), smarter gating, solopreneur framing.
 *
 * Only idea-worthy emails get ideas. The model can return 0 ideas when
 * the email doesn't warrant any (transactional, automated, thin content).
 *
 * DESIGN PHILOSOPHY:
 * - Not every email deserves ideas — a receipt doesn't need 3 ideas forced onto it
 * - Ideas should be lateral, not obvious
...
export interface IdeaSparkData {
   ...
   * False for emails that don't warrant ideas or very short/empty emails.
   */
  hasIdeas: boolean;

  /**
   * Array of 0-3 ideas inspired by this email.
   * Each idea is typed, explained, and confidence-scored.
   * May be empty when the email doesn't warrant creative ideas.
   */
  ideas: IdeaSpark[];

  /**
   * Why ideas were skipped (only present when hasIdeas is false).
   * Helps debugging and explains to the user why no ideas were generated.
   */
  skipReason?: string;
...
export interface AggregatedAnalysis {
   ...
   * Contains 0-3 creative ideas inspired by the email + user context.
   * Only populated for idea-worthy emails (not transactional/automated).
   *
   * Ideas span: tweet drafts, networking, business, learning, tools to try,
   * places to visit, date nights, family activities, personal growth, community.
   */
