...
| Dates & Hidden Fields Phase 1 | Mar 2026 | **Surface extracted dates + hidden email metadata** (4 sub-tasks). **1A Calendar dates**: Updated `event-colors.ts` — deadline→red/AlertTriangle, payment_due→red/circle, expiration→amber/Clock, follow_up→amber, reminder→purple/Bell, appointment→blue/Calendar. Created `DateItemExpanded.tsx` for non-event date types (slim card with description, source email link, "Create task" placeholder). `TimelineItem.tsx` conditionally renders DateItemExpanded for non-event types (deadline, payment_due, expiration, follow_up, reminder, recurring, other). Added `sourceEmailSubject` to CalendarItem type. Calendar page filters out `date_type='event'` from extracted dates to avoid duplicates with events from `/api/events`. **1B Triage expansion**: Extended `TriageItem` type with `'deadline' | 'event'`. `deadlineToTriageItem()` and `eventToTriageItem()` normalizers with urgency by date proximity (today=10, tomorrow=8, 3d=6, 7d=4). `useTriageItems` fetches extracted dates (deadline/payment_due/expiration within 7 days, not acknowledged) and events (commitment_level=invited within 7 days). Created `TriageDeadlineCard.tsx` (red AlertTriangle, "Create task" primary action) and `TriageEventCard.tsx` (purple Calendar, RsvpBadge, "Add to calendar" primary action). `TriageContent.tsx` extended with new card types, filter pills (All/Tasks/Ideas/Deadlines/Events), quick accept handlers with `due_date`. Stats: `{ total, actions, ideas, deadlines, events }`. **1C Inbox hidden fields**: Timeliness date chip on inbox rows — "Stale after Mar 10" (amber/Clock) or "Expires Mar 15" (red/AlertTriangle) from `email.timeliness` JSONB, future dates only. TODO comments for contact company in sender line (needs contacts join) and must-read link indicator (needs url_extraction data). **1D Email detail**: `useContacts` hook fetches sender contact by email, displays job_title + company below sender email ("Product Manager at Acme Corp"). `TimelinessBanner` component shows late_after/expires with timeliness accent colors for future dates, red for past dates. 3 new files, 6 modified files. |
